...
public class GeneralStatStorage implements StatMySQLHandler {
    ...
    public Collection<StatTimeEntry> loadEntries(Connection con, UUID uuid) throws SQLException {
        ...
        try (PreparedStatement st = con.prepareStatement("SELECT world_id, amount, last_updated " +
    ...
    public void storeEntry(Connection con, MySQLStatsPlayer player, StatsContainer container, StatTimeEntry entry) throws SQLException {
        try (PreparedStatement st = con.prepareStatement("INSERT INTO " + getTableName() + " (player_id, world_id, amount) " +
